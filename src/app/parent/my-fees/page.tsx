  ...
  useEffect(() => {
    const fetchFees = async () => {
      ...
      try {
        ...
        const feesData = feesSnap.docs.map(
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage and track your ward's fee payments.
          </p>
        </div>
        <button
          ...
          {fees.filter((f) => f.status === "pending").length > 0 ? (
            <>
              <p className="text-xl font-semibold mt-2 text-gray-900">
                {formatDate(
                  fees.filter((f) => f.status === "pending")[0]?.dueDate,
                )}
              </p>
              <p className="text-sm text-gray-500">
                Plan payments to avoid fines.
              </p>
            </>
          ) : (
            <>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50">
          <p className="text-xs text-gray-500 text-center">
            Payments typically process within 1-3 business days. For issues,
            contact accounts@edumate.edu
          </p>
        </div>
      </div>
    </div>
