          ...
          <>
            ...
            <div className="bg-white rounded-lg border border-gray-100 p-6 mb-6">
              ...
              <p className="text-sm text-gray-600 mt-4">
                {overallPct >= 75
                  ? "Great! Your child's attendance is above the 75% requirement."
                  : "Your child's attendance is below the 75% requirement. Please encourage them to attend more classes."}
              </p>
            </div>
                    ...
                    return (
                      <div key={record.id} className={`border-l-4 ${isCritical ? 'border-red-400 bg-red-50' : 'border-yellow-400 bg-yellow-50'} p-4 rounded-r-lg`}>
                        <p className={`text-sm font-medium ${isCritical ? 'text-red-800' : 'text-yellow-800'}`}>
                          {isCritical ? 'Critical Alert' : 'Warning'} — {record.subject}: {pct.toFixed(1)}% (Below 75%). Please take action.
                        </p>
                      </div>
                    );
                  })}
                  ...
                  return (
                    <div key={day}
                      ...
                      {records.map(rec => {
                        ...
                        return (
                          <div key={rec.docId} className={`${cfg.bg} border ${cfg.border} rounded px-1.5 py-0.5`}>
