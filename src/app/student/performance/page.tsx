...
export default function PerformancePage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      if (data.length > 0) {
        ...
        const calculatedGpa = parseFloat(
          (totalPoints / data.length).toFixed(2),
        );
        setGpa(isNaN(calculatedGpa) ? 0 : calculatedGpa);

        const tCredits = data.reduce(
  ...
  const gpaTrend = semLabels.map((sem) =>
    (semesterGroups[sem].sum / semesterGroups[sem].count).toFixed(2),
  );
  ...
  const subScores = subLabels.map((sub) =>
    Math.round(subjectGroups[sub].sum / subjectGroups[sub].count),
  );
  ...
  evaluations.forEach((ev) => {
    if (
      ...
      new Date(ev.date) > new Date(latestScores[ev.subject].date)
    ) {
