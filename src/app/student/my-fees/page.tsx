...
export default function FeesPage() {
  ...
  useEffect(() => {
    const fetchFees = async () => {
      ...
      try {
        ...
        const feesData = snapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() }) as Fee,
        );
  ...
  const seedFeesData = async () => {
    ...
    try {
      const sampleFees = [
        {
          item: "Tuition Fee - Semester 5",
          amount: 45000,
          status: "pending",
          dueInDays: 30,
        },
        {
          item: "Lab Fee - Computer Science",
          amount: 5000,
          status: "pending",
          dueInDays: 15,
        },
  ...
  const handlePay = async (fee: Fee) => {
    ...
    setTimeout(async () => {
      try {
        ...
        setFees((prev) =>
          prev.map((f) =>
            f.id === fee.id ? { ...f, status: "paid", paidAt: new Date() } : f,
          ),
        );
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Track your financial status and payment history.
          </p>
        </div>

        {process.env.NODE_ENV === "development" && fees.length === 0 && (
          <button
            onClick={seedFeesData}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-gray-100 flex flex-col justify-between">
          ...
          <div className="mt-4 w-full bg-gray-100 rounded-full h-1.5">
            <div
              ...
              style={{ width: totalOutstanding > 0 ? "60%" : "0%" }}
            ></div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 flex flex-col justify-between">
          <div>
            ...
            <p className="text-xl font-bold text-gray-900 mt-2">
              {nextDueDate
                ? new Date(nextDueDate.seconds * 1000).toLocaleDateString(
                    undefined,
                    {
                      day: "numeric",
                      month: "long",
        ...
        <div className="bg-blue-600 p-6 rounded-2xl text-white flex flex-col justify-between">
          <div>
            <h2 className="text-blue-100 text-sm font-medium uppercase tracking-wide">
              Secure Payment
            </h2>
            <p className="text-sm text-blue-50 mt-2 opacity-90">
              All transactions are encrypted and secured. We support UPI, Cards,
              and Netbanking.
            </p>
          </div>
        </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-100">
                {fees.map((fee) => {
                  ...
                  const date = fee.dueDate?.seconds
                    ? new Date(fee.dueDate.seconds * 1000)
                    : new Date();
                      ...
                      <td className="p-4 text-right">
                        ...
                        {fee.status === "paid" && (
                          <span className="text-gray-400 text-sm">
                            Paid on {fee.paidAt?.toDate().toLocaleDateString()}
                          </span>
                        )}
                      </td>
                    </tr>
