      ...
      <div className="w-full">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Manage user access levels and permissions across the system.
          </p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {users.map((user) => {
                  const roleInfo =
                    ROLES_CONFIG[user.role as keyof typeof ROLES_CONFIG] ||
                    ROLES_CONFIG["Faculty"];
                  return (
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${user.accessStatus === "active" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                        >
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            {Object.entries(ROLES_CONFIG).map(([role, config]) => (
              <div key={role} className="border border-gray-200 rounded-lg p-4">
                <h3 className={`font-bold mb-2 ${config.color.split(" ")[1]}`}>
                  {role}
                </h3>
                <div className="mb-2">
