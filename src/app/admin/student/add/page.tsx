...
export default function AddStudent() {
  ...
  const [formData, setFormData] = useState({
    ...
    uniRegNumber: "", // University Register Number
    email: "",
    password: "",
    phone: "",
    department: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const q = query(
        studentsRef,
        where("regNumber", "==", formData.regNumber),
      );
      const querySnapshot = await getDocs(q);

      if (!querySnapshot.empty) {
        throw new Error(
          `Register Number ${formData.regNumber} already exists!`,
        );
      }
      ...
      secondaryApp =
        foundApp || initializeApp(firebaseConfig, secondaryAppName);
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              onClick={async () => {
                ...
                if (!formData.department || !formData.batch) {
                  toast.error("Select Department and Batch to generate ID", {
                    icon: "⚠️",
                  });
                  return;
                }
                try {
                  let deptCode = formData.department
                    .substring(0, 3)
                    .toUpperCase();
                  if (formData.department.toLowerCase().includes("computer"))
                    deptCode = "CS";
                  if (formData.department.toLowerCase().includes("mechanical"))
                    deptCode = "ME";
                  if (formData.department.toLowerCase().includes("civil"))
                    deptCode = "CE";
                  if (formData.department.toLowerCase().includes("electrical"))
                    deptCode = "EEE";
                  if (formData.department.toLowerCase().includes("electronics"))
                    deptCode = "ECE";

                  const yearMatch = formData.batch.match(/\d{4}/);
                  ...
                  // Find highest incremental number (simplified logic)
                  let maxNum = 1000;
                  snapshot.docs.forEach((doc) => {
                    const reg = doc.data().regNumber || "";
                    const parts = reg.split("/");
                    if (parts.length > 0 && !isNaN(parseInt(parts[0]))) {
                      maxNum = Math.max(maxNum, parseInt(parts[0]));
                    }
                  });

                  const uniqueId = `${maxNum + 1}/${deptCode}/${year}`;
                  setFormData((prev) => ({ ...prev, regNumber: uniqueId }));
                  toast.success(`Generated ID: ${uniqueId}`);
                } catch (e) {
                  toast.error("Failed to generate ID");
                }
              }}
              className="text-sm font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg transition-colors border border-blue-100"
            >
              Generate Format ID
            </button>
            <Link
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="text-xs text-blue-600 mt-1">
                Select Department & Batch to auto-generate (Top Right)
              </p>
            </div>
            <div>
              {/* Roll Number */}
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Min. 6 characters. Encrypted by Firebase.
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
